...
const BlogCard = ({ id, slug, title, excerpt, imageUrl, content, showActions, onDelete, onEdit }: BlogPostProps) => {
      ...
      <div className="bg-white rounded-xl p-6 md:p-8 shadow-sm hover:shadow-md transition-all duration-300 border border-transparent w-full mb-6 relative overflow-hidden flex flex-col md:flex-row gap-6 items-start">
        ...
        {showActions && (
          <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
              <button 
                  onClick={() => onEdit({ id, title, content: content || excerpt, imageUrl })} 
                  className="p-2 bg-blue-50 text-blue-600 rounded-full hover:bg-blue-100 shadow-sm border border-blue-200"
                  title="Editar"
              >
                  <Edit size={18} />
              </button>
              <button 
                  onClick={() => onDelete(id)} 
                  className="p-2 bg-red-50 text-red-600 rounded-full hover:bg-red-100 shadow-sm border border-red-200"
                  title="Deletar"
              >
                  <Trash2 size={18} />
              </button>
